...
export function listProducts(eventId: number, includeInactive = true): Product[] {
...
export function createProduct(eventId: number, input: { name: string; price: number; category_id?: number | null; icon?: string; color?: string; sort_order?: number }, userId: number) {
  ...
  audit(userId, 'create', 'product', res.lastInsertRowid, { event_id: eventId, name, price });
  return getProduct(res.lastInsertRowid)!;
}

export function updateProduct(id: number, input: Partial<{ name: string; price: number; category_id: number | null; icon: string; color: string; sort_order: number; active: number }>, userId: number) {
  ...
  audit(userId, 'update', 'product', id, { name: input.name, price });
  return getProduct(id)!;
}

export function deleteProduct(id: number, userId: number) {
  const used = getRow<{ c: number }>('SELECT COUNT(*) AS c FROM sale_items WHERE product_id = ?', id);
  if (used!.c > 0) {
    throw BadRequest('No se puede eliminar un producto ya vendido. Podés desactivarlo.');
  }
  exec('DELETE FROM products WHERE id = ?', id);
  audit(userId, 'delete', 'product', id, {});
  return true;
}

export function duplicateProduct(id: number, userId: number) {
  const cur = getProduct(id);
  if (!cur) throw BadRequest('Producto no encontrado');
  const res = exec(
  ...
  audit(userId, 'duplicate', 'product', res.lastInsertRowid, { from: id });
  return getProduct(res.lastInsertRowid)!;
}