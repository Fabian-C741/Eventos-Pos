...
export function listTicketTypes(eventId: number): TicketType[] {
...
export function getTicketType(id: number): TicketType | undefined {
  return getRow<TicketType>('SELECT * FROM ticket_types WHERE id = ?', id);
}

export function createTicketType(eventId: number, input: { name: string; price: number; kind: TicketKind; start_number?: number | null; digits?: number; icon?: string; color?: string }, userId: number) {
...
const id = res.lastInsertRowid;
  audit(userId, 'create', 'ticket_type', id, { event_id: eventId, name, price, kind: input.kind });
  return getTicketType(id)!;
}

export function updateTicketType(id: number, input: Partial<{ name: string; price: number; kind: TicketKind; start_number: number | null; digits: number; icon: string; color: string; active: number }>, userId: number) {
  const cur = getTicketType(id);
  if (!cur) throw BadRequest('Tipo no encontrado');
  const price = input.price !== undefined ? Math.round(Number(input.price)) : cur.price;
  if (isNaN(price) || price < 0) throw BadRequest('El precio debe ser un número válido');
  const sold = getRow<{ c: number }>('SELECT COUNT(*) AS c FROM tickets WHERE ticket_type_id = ?', id);
  const start = input.start_number !== undefined ? input.start_number : cur.start_number;
  let last = cur.last_number;
  if (input.start_number !== undefined && sold!.c === 0) {
  ...
  audit(userId, 'update', 'ticket_type', id, { name: input.name, price });
  return getTicketType(id)!;
}

export function deleteTicketType(id: number, userId: number) {
  const sold = getRow<{ c: number }>('SELECT COUNT(*) AS c FROM tickets WHERE ticket_type_id = ?', id);
  if (sold!.c > 0) {
    throw BadRequest('No se puede eliminar un tipo de entrada ya vendido. Podés desactivarlo.');
  }
  exec('DELETE FROM ticket_types WHERE id = ?', id);
  audit(userId, 'delete', 'ticket_type', id, {});
  return true;
}

export function allocateTicketNumbers(ticketTypeId: number, quantity: number): { type: TicketType; numbers: number[] } {
  const type = getTicketType(ticketTypeId);
  ...
  exec('UPDATE ticket_types SET last_number = ? WHERE id = ?', start + quantity - 1, ticketTypeId);
  return { type, numbers };
}

export function lastTicketNumbers(eventId: number) {
